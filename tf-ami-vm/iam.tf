...
output "tpiac_users" {
  ...
  value = [
    {
    "user_name" = aws_iam_user.tpiac[*].name
    "user_pwd"  = aws_iam_user_login_profile.tpiac[*].password
    "user_apikey" = aws_iam_access_key.tpiac[*].id
    "user_apikey_secret" = aws_iam_access_key.tpiac[*].secret
    }
  ]
}
