...
for REGION in eu-central-1 eu-west-1 eu-west-2 eu-west-3 eu-south-1 eu-south-2 eu-north-1 eu-central-2; do
  ...
  INSTANCES=$("$AWS_CLI" ec2 describe-instances --region "$REGION" --output json)
    ...
    # VPCs
    "$AWS_CLI" ec2 describe-vpcs --region "$REGION" --output json | jq -r --arg region "$REGION" '
    .Vpcs[] |
    "aws_vpc{region=\"" + $region + "\", vpc_id=\"" + .VpcId + "\", cidr=\"" + .CidrBlock + "\"} 1"
    ' >> "$TMP_FILE"

    # Internet Gateways
    "$AWS_CLI" ec2 describe-internet-gateways --region "$REGION" --output json | jq -r --arg region "$REGION" '
    .InternetGateways[] |
    "aws_internet_gateway{region=\"" + $region + "\", igw_id=\"" + .InternetGatewayId + "\"} 1"
    ' >> "$TMP_FILE"

    # Security Groups
    "$AWS_CLI" ec2 describe-security-groups --region "$REGION" --output json | jq -r --arg region "$REGION" '
    .SecurityGroups[] |
    "aws_security_group{region=\"" + $region + "\", sg_id=\"" + .GroupId + "\", name=\"" + .GroupName + "\"} 1"
    ' >> "$TMP_FILE"

    # EIPs
    "$AWS_CLI" ec2 describe-addresses --region "$REGION" --output json | jq -r --arg region "$REGION" '
    .Addresses[] |
