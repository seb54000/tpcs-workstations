...
do
  ...
  echo "VM : knode${digits}.tpcs.tpcsonline.org"
  echo "Connect to 'node' VM to launch join command"
  ${ssh_quiet} -o ConnectTimeout=5 -o ConnectionAttempts=1 -i $(dirname "$0")/../key vm${digits}@knode${digits}.tpcs.tpcsonline.org ''microk8s join ${JOIN_CMD} --worker''
