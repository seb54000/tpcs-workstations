...
do
  ...
  do
    ...
    STATUS=$(${ssh_quiet} -i $(dirname "$0")/../key -o ConnectTimeout=5 -o ConnectionAttempts=1 cloudus@${VM_FQDN} 'sudo cloud-init status')
    ...
    # echo -e "Connected (with SSH) to VM : $(ssh-quiet -i $(pwd)/key cloudus@${VM_FQDN} 'hostname')"
    # ssh-quiet -i $(dirname "$0")/key cloudus@${VM_FQDN} 'cat /home/cloudus/user_data_common_finished 2&> /dev/null && echo "cloudinit finished" || echo "cloudinit still ongoing"'
  done
done
